...
const harmonyGenerator = (colorCount) => {
    ...
    const colors = Array(colorCount).fill(0).map(() => {
        ...
        if (Math.abs(referenceAngle + randomAngle - previousAngle) < 30)
            randomAngle += (0, util_1.randomInt)(20, 30);
        const newColor = new color_1.Color([(referenceAngle + randomAngle) % 360, Math.random() * (1 - saturation) + saturation, Math.random() * 0.3 + 0.5], 'hsl');
